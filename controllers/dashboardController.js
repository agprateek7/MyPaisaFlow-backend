...
exports.getDashboardData = async (req, res) => {
    try {
        ...
        const lastIncome = (await Income.find({ userId }).sort({ date: -1 }).limit(5)).map(txn => ({
            ...txn.toObject(),
            type: 'income'
        }));
        const lastExpense = (await Expense.find({ userId }).sort({ date: -1 }).limit(5)).map(txn => ({
            ...txn.toObject(),
            type: 'expense'
        }));
        const lastTransactions = [...lastIncome, ...lastExpense].sort((a, b) => b.date - a.date).slice(0, 5);
        ...
        res.json({
            totalBalance: 
                ...
                recentTransactions: lastTransactions // Limit to 5 recent transactions
        })
    } catch (error) {
